...
* `add`: Add a named camera instance 
* `add-scenario`: Add a scenario for a camera 
...
* `preview`: Preview the camera system View 
* `remove`: Remove a camera from the system 
* `remove-scenario`: Remove a scenario from a camera 
...
Show details of a pipeline. Show what is configured in the current pipeline.
```console
$ visionai pipeline --name test_pipe show
```
@arg pipeline - specify a named pipeline
